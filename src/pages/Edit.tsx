...
const Edit = () => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (file) {
        // image 오버라이트 후 doc 업데이트
        const locationRef = ref(storage, `${user.uid}/tickets/${docId}`);
  ...
  return (
    <Wrapper>
      <Header
        center={"일정 수정"}
        right={
          <input className="button" type="submit" value="저장" form="ticket" />
        }
      />
      <main>
